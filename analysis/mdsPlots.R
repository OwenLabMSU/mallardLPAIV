...
## Load data
cnt.bursa <- read.table("G:/Shared drives/RNA Seq Supershedder Project/MALL DE manuscript/all.genes.results.csv", header = TRUE) %>%
  as_tibble(rownames = NA)
cnt.ileum <- read.csv("G:/Shared drives/RNA Seq Supershedder Project/MALL DE manuscript/degnorm_08272020_162816/adjusted_read_counts.csv", header = TRUE) %>%
  as_tibble(rownames = NA)
covars <- read_delim("G:/Shared drives/RNA Seq Supershedder Project/MALL DE manuscript/MALL70_SSgroup_RAW_7.7.20.csv", delim = ",")
annot.bursa <- read.delim("G:/Shared drives/RNA Seq Supershedder Project/MALL DE manuscript/annotationGenes52820.txt", header = TRUE, sep = "\t")
...
newNames.ileum <- names(cnt.ileum.tib) %>%
  as.tibble() %>%
  separate(value, into = c("pt1", "pt2", "pt3", "pt4")) %>%
  select(pt2, pt3) %>%
  mutate(pt2 = na_if(pt2, "ileum"),
         pt3 = na_if(pt3, "ileum")) %>%
  mutate(bird = ifelse(is.na(pt2), pt3, pt2)) %>%
...
#### Determine number of axes to retain
...
summary(varExplained.ileum) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F)

#### Construct robust PCA outlier map
